...
<script type="text/javascript">
	...
	function fnFileCheck(){
		$('origin').change(function(){
			...
			let fileSize = $(this)[0].files[0].size;	//첨무된 파일 크기
		});
	}
	...
	function fnAddBoard(){
		$('#insert_btn').click(function(){
			...
			//ajax의 파일 업로드는 가상 form사용 (form 객체)
			var formData = new FormData();
			formData.append('origin', $('#origin')[0].files[0]);	//첨부를 FormData에 넣기
			$.ajax({
					url:'/ex15/api/boards',
					type:'post',
					contentType: false,		//ajax파일 첨부할때 세팅
					processDate: false,		//ajax파일 첨부할때 세팅
					data:formData,
					dataType:'json',
					success: function(){
					}
...
<body>
	...
	<div>
		<form id="f">
			...
			첨부 <input type="file" name="origin" id="origin" multiple><br>
